defmodule Litmus.Pure do
  ...
  - Checks **direct function calls** and **captured functions** in the AST
  - Cannot detect impurity from:
    - Dynamic dispatch (`apply/3`)
    - Functions passed as variables
    - Local function captures (`&local_function/arity`) - requires module context
  ...
  # Extract function calls from an AST (for anonymous function captures)
  defp extract_calls_from_ast(ast) do
    ...
    case calls do
      ...
      multiple ->
        # If there are multiple calls, we can't determine which one
        # represents the "effect" of the anonymous function, so we'll
        # return the first one as a conservative approach
        hd(multiple)
    end
  end
